...
export default function AppSpreading() {
  ...
  const results = useMemo((): CalculationResults => {
    ...
    // Mt = Carico massimo Azoto / N tot (g/L)
    // Since N tot is g/L, and we want Mt in tons (m3), 170 kg / (X g/L) = 170 / X m3
    ...
    // Ms = Mt / (1 + R * VSs / VSi)
    // Mt is in tons, we convert to kg for Ms/Mi
  ...
  const handleInputChange = (
    ...
    setter((prev: any) => {
      ...
      // Auto-update VS Sludge if TS Sludge changes (75% rule)
      if (field === 'tsSludge') {
      ...
      <main className="flex-1 p-6 max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <motion.div 
            ...
            className="bg-brand-primary text-white rounded-3xl p-8 shadow-xl relative overflow-hidden"
          >
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-10 pt-8 border-t border-white/10">
                <div>
                  <span className="block text-[10px] text-slate-500 font-bold uppercase mb-1">Digestato Tot.</span>
                  <span className="text-lg font-mono font-bold">{results.mtTons.toFixed(1)} <span className="text-xs font-normal opacity-50">t</span></span>
                </div>
                <div>
                  <span className="block text-[10px] text-slate-500 font-bold uppercase mb-1">Massa Fango</span>
                ...
                <div>
                  <span className="block text-[10px] text-slate-500 font-bold uppercase mb-1">Interventi</span>
                  <span className="text-lg font-mono font-bold">{results.minInterventions.toFixed(1)} <span className="text-xs font-normal opacity-50">min</span></span>
                </div>
                <div>
                  <span className="block text-[10px] text-slate-500 font-bold uppercase mb-1">Massa Inoculo</span>
                  <span className="text-lg font-mono font-bold">{Math.round(results.miKg).toLocaleString()} <span className="text-xs font-normal opacity-50">kg</span></span>
                </div>
              </div>
            </div>
          </motion.div>
            ...
            <div className="glass-card p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Variazione Indice Conversione 0.8 - 1.2</p>
                </div>
              ...
              <div className="h-48 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={sensitivityFCR}>
                    ...
                    <YAxis axisLine={false} tickLine={false} tick={{fontSize: 10}} />
                    <Tooltip contentStyle={{ borderRadius: '12px', border: 'none' }} />
                    <Area type="monotone" dataKey="biomass" stroke="#a855f7" fill="url(#colorFCR)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="glass-card p-6 flex flex-col">
              ...
              <div className="space-y-4 flex-1">
                <div className="flex gap-3">
                  <div className="w-1 h-auto bg-accent rounded-full shrink-0" />
                  <p className="text-xs text-slate-600 leading-relaxed">
                    La riduzione del rapporto di co-digestione <strong>R</strong> (maggiore impiego di fango rispetto all'inoculo) ottimizza la sostenibilità del sistema.
                  </p>
                </div>
                <div className="flex gap-3">
                  <div className="w-1 h-auto bg-blue-500 rounded-full shrink-0" />
                  <p className="text-xs text-slate-600 leading-relaxed">
                    Il limite di spandimento è calcolato sulla base della <strong>Zona Vulnerabile da Nitrati (ZVN)</strong> fissata a 170 kg N/ha/anno.
                  </p>
                </div>
                <div className="flex gap-3">
                  <div className="w-1 h-auto bg-orange-500 rounded-full shrink-0" />
                  <p className="text-xs text-slate-600 leading-relaxed">
                    I parametri <strong>P</strong> e <strong>FCR</strong> sono determinanti per la conversione dei solidi totali in biomassa ittica effettiva.
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-3">
                ...
                <p className="text-[10px] text-slate-500 font-medium">
                  Assicurarsi che i dati TS ed Azoto derivino da test BMP certificati.
                </p>
              </div>
            </div>
          </div>
